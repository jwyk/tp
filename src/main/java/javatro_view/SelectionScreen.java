...
/**
 * The {@code SelectionScreen} class represents an abstract screen where users
 * select cards from their hand. It provides methods for updating and displaying
 * the player's current hand. (Used for the SelectCardsToDiscard and SelectCardsToPlay)
 */
public class SelectionScreen extends Screen {
    ...
    protected void displayCardsInHoldingHand() {
        ...
        for (int i = 0; i < holdingHand.size(); i++) {
            numberHeaders += "|" + GameScreen.getDisplayStringCenter(Integer.toString(i + 1), 5) + "|    ";
        }
        ...
        String cardValues = "";
        for (int i = 0; i < holdingHand.size(); i++) {
            cardValues += "|" + GameScreen.getDisplayStringCenter(holdingHand.get(i).getSimplified(), 5) + "|    ";
        }
