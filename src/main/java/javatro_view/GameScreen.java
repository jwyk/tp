...
public class GameScreen extends Screen{
  ...
  public void displayGameScreenUI() {
    ...
    System.out.printf("|  [● %-10s BLIND]  Score: %-5d            |\n", blindDisplay, Math.min(10, 99999));

    System.out.println("|------------------------------------------------|");

    System.out.printf("|  Round Score: ○%-6d                          |\n", Math.min(roundScore, 999999));
    System.out.println("|------------------------------------------------|");

    System.out.println("|  Run Info                                      |");

    // Ensure handsLeft and discardCount fit within space
    System.out.printf("|  Hands: %-2d       Discards: %-2d                  |\n", Math.min(handsLeft, 99), Math.min(discardCount, 99));

    // Format money
    String moneyDisplay = moneyInfo >= 1_000_000 ? String.format("$%dM", moneyInfo / 1_000_000)
        : moneyInfo >= 1000 ? String.format("$%dK", moneyInfo / 1000)
